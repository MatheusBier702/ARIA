...
SYSTEM_NAME = "A.R.I.A"
MODEL_BADGE = "AUTONOMOUS RESPONSIVE INTELLIGENCE ASSISTANT"

# ── Renk paleti ── ARIA monochrome ────────────────────────────────────────────
C_BG      = "#000000"
C_PRI     = "#f2f2f2"
C_ORG     = "#e8e8e8"
C_ORG2    = "#ffffff"
...
class JarvisUI:
    def __init__(self):
        ...
        # ── Health overlay ───────────────────────────────────────────────────
        self._last_response_text = "Claro! Como posso te ajudar?"
        self._last_response_time = time.strftime("%H:%M")
    ...
    def _set_layout_metrics(self, width: int, height: int):
        ...
        self.FCY = HDR_H + orb_area_h // 2 - 4
        self.FACE = min(int(orb_area_h * 0.92), int(center_w * 0.62), 620)
        ...
        # ── Alt orta sütun: durum etiketi · dalga · son yanıt kartı · mikrofon ──
        self.STATE_Y = self.CTRL_Y + 18
        self.WAVE_Y  = self.STATE_Y + 26
        self.RESP_X  = self.CTRL_X
        self.RESP_Y  = self.WAVE_Y + 20
        self.RESP_W  = self.CTRL_W
        self.RESP_H  = 92
        self.MIC_Y   = self.RESP_Y + self.RESP_H + 34
        self.MIC_R   = 44

        # ── Sağ panel: ESTADO / VOLUME üstte, konuşma geçmişi altta ─────────
        ...
        self.STATUS_PANEL_H = 168
        self.CHAT_X = self.CHAT_PANEL_X + 10
        self.CHAT_Y = self.CHAT_PANEL_Y + self.STATUS_PANEL_H + 14
        self.CHAT_W = self.CHAT_PANEL_W - 20
        self.CHAT_H = self.CHAT_PANEL_H - self.STATUS_PANEL_H - 14 - 56
    ...
    def _build_social_bar(self):
        ...
        name_lbl.bind("<Button-1>", _open("https://github.com/MatheusBier702"))
    ...
    # ── Encerrar (sol alt, sade metin linki) ─────────────────────────────────
    def _build_shutdown_button(self):
        BW, BH = self.LEFT_W - 28, 30
    ...
    def _draw_shutdown_button(self):
        ...
        BH = int(c["height"])
        c.delete("all")
        c.create_text(4, BH // 2, text="⏻", fill=C_RED,
                      font=(FONT_BODY_FAMILY, 13), anchor="w")
        c.create_text(26, BH // 2, text="ENCERRAR", fill="#8a8a8a",
    ...
    def _build_settings_panel(self):
        ...
        self._settings_body = tk.Frame(self._settings_panel, bg="#0a0a0a")
        self._debug_body = tk.Frame(self._settings_panel, bg="#0a0a0a")
    ...
    def _draw_settings_button(self):
        ...
        bh = int(c["height"])
        c.delete("all")
        accent = C_PRI if self._settings_open else "#8a8a8a"
        c.create_text(4, bh // 2, text="⚙", fill=accent,
                      font=(FONT_BODY_FAMILY, 13), anchor="w")
        c.create_text(26, bh // 2, text="CONFIGURAÇÕES", fill=accent,
                      font=font_body_bold(10), anchor="w")
    ...
    def _draw_settings_tabs(self):
        for key, canvas, label in (
            ...
            fill = "#082020" if active else "#0a0a0a"
    ...
    def _build_mute_button(self):
        size = self.MIC_R * 2 + 60
        self._mute_canvas = tk.Canvas(self.root, width=size, height=size,
                                      bg=C_BG, highlightthickness=0, cursor="hand2")
    ...
    def _draw_mute_button(self):
        ...
        bh = int(c["height"])
        c.delete("all")
        col = C_MUTED if self.muted else C_TEXT
        icon = "🔇" if self.muted else "🎙"
        cx = bw // 2
        cy = self.MIC_R + 4
        r = self.MIC_R
        # outer soft ring
        ring_rgb = (255, 51, 68) if self.muted else (240, 240, 240)
        c.create_oval(cx - r - 8, cy - r - 8, cx + r + 8, cy + r + 8,
                      outline=self._ac(ring_rgb[0], ring_rgb[1], ring_rgb[2], 40), width=1)
        c.create_oval(cx - r, cy - r, cx + r, cy + r,
                      fill="#0a0a0a", outline=col, width=2)
        c.create_text(cx, cy, text=icon, fill=col, font=("Segoe UI Emoji", int(r * 0.75)))
        c.create_text(cx, cy + r + 26, text=("MIC MUDO" if self.muted else "MIC ATIVO"),
                      fill="#9a9a9a", font=font_body_bold(10))
    ...
    def _draw_pause_button(self):
        ...
        c.delete("all")
        if self.paused:
            col, glyph, text = C_GOLD, "▶", "RETOMAR"
        else:
            col, glyph, text = "#8a8a8a", "⏸", "PAUSAR"
        c.create_text(4, bh // 2, text=glyph, fill=col,
                      font=(FONT_BODY_FAMILY, 13), anchor="w")
        c.create_text(26, bh // 2, text=text, fill=col,
                      font=font_body_bold(10), anchor="w")
    ...
    def _place_layout_widgets(self):
        ...
        # ── Sol kenar çubuğu alt köşe: Ayarlar / Pause / Encerrar ───────────
        link_x = 20
        shutdown_h = int(self._shutdown_canvas["height"])
        pause_h = int(self._pause_canvas["height"])
        btn_h = int(self._settings_btn_canvas["height"])
        shutdown_y = self.H - FOOTER_H - shutdown_h - 18
        pause_y = shutdown_y - pause_h - 10
        settings_y = pause_y - btn_h - 10
        self._shutdown_canvas.place(x=link_x, y=shutdown_y)
        self._pause_canvas.place(x=link_x, y=pause_y)
        self._settings_btn_canvas.place(x=link_x, y=settings_y)

        # ── Büyük mikrofon düğmesi (alt orta) ────────────────────────────────
        mic_size = int(self._mute_canvas["width"])
        self._mute_canvas.place(x=self.FCX - mic_size // 2, y=self.MIC_Y)

        geo = self._settings_geometry
        panel_w = geo["panel_w"]
        panel_h = geo["panel_h"]
        panel_x = link_x
        panel_y = settings_y - panel_h - 10
    ...
    def write_log(self, text: str):
        ...
        if tl.startswith("siz:") or tl.startswith("you:"):
            ...
            self._last_response_text = body.strip()
            self._last_response_time = time.strftime("%H:%M")
    ...
    # ── ARIA görsel yardımcıları ─────────────────────────────────────────────
    def _rounded_rect(self, c, x0, y0, x1, y1, r, **kwargs):
        points = [
    ...
    def _draw_status_pill(self, c, right_x, cy, text, dot_col):
        pad_x = 14
        w = 14 + len(text) * 8 + pad_x * 2
        h = 26
        x1 = right_x
        x0 = x1 - w
        y0 = cy - h // 2
        y1 = cy + h // 2
        self._rounded_rect(c, x0, y0, x1, y1, h // 2,
                           fill="#0c0c0c", outline="#2a2a2a", width=1)
        blink = 255 if self.status_blink or dot_col != C_GREEN else 120
        dr = 4
        c.create_oval(x0+pad_x-dr, cy-dr, x0+pad_x+dr, cy+dr,
                      fill=self._ac(*self._hex_to_rgb(dot_col), blink), outline="")
        c.create_text(x0+pad_x+dr*2+6, cy, text=text, fill="#e6e6e6",
    ...
    def _draw_wave_dots(self, c, cx, cy, width, color, small=False):
        n = 13 if not small else 7
        gap = width / (n - 1)
        x0 = cx - width / 2
        t = self.tick
        ...
        for i in range(n):
            ...
            mid = n // 2
            dist = abs(i - mid)
            if active and dist <= 3:
                amp = (4 + dist * 2) + abs(math.sin(t * 0.25 + i * 0.6)) * (10 - dist * 2)
            else:
                amp = 2
            c.create_line(x, cy - amp, x, cy + amp, fill=color, width=2, capstyle="round")

    def _draw_response_card(self, c):
        x0, y0 = self.RESP_X, self.RESP_Y
        w, h = self.RESP_W, self.RESP_H
        self._rounded_rect(c, x0, y0, x0 + w, y0 + h, 14,
                           fill="#050505", outline="#1c1c1c", width=1)
        c.create_text(x0 + 20, y0 + 20, text="ÚLTIMA RESPOSTA",
                      fill="#6e6e6e", font=font_body_bold(9), anchor="w")
        c.create_text(x0 + w - 20, y0 + 20, text=self._last_response_time,
                      fill="#5a5a5a", font=font_body(9), anchor="e")
        msg = self._last_response_text or "..."
        if len(msg) > 92:
            msg = msg[:89] + "..."
        c.create_text(x0 + 20, y0 + 48, text=msg, fill="#f0f0f0",
                      font=font_body(13), anchor="nw", width=w - 40, justify="left")
    ...
    def _draw_left_panel(self, c):
        ...
        c.create_text(x0, y0, text="SISTEMA", fill="#7a7a7a",
                      font=font_body_bold(11), anchor="w")

        # ── CPU / RAM dairesel göstergeler ───────────────────────────────────
        gauge_r = min(58, int(pw * 0.32))
        gap_between = 26
        gy = y0 + 48 + gauge_r
        gx1 = x0 + gauge_r
        gx2 = x0 + gauge_r * 2 + gap_between + gauge_r
        self._draw_gauge(c, gx1, gy, gauge_r, self._stats['cpu'], "CPU")
        self._draw_gauge(c, gx2, gy, gauge_r, self._stats['ram'], "RAM")

        cy = gy + gauge_r + 34
        c.create_line(x0, cy, x0 + pw, cy, fill="#161616", width=1)
        cy += 22

        # ── Disk / Pil / Ağ ────────────────────────────────────────────────
        for label, key, unit in [("DISK", "disk", "%"), ("BATERIA", "battery", "%")]:
            val = self._stats[key]
            col = "#e6e6e6"
            if key == "battery" and val < 20:
                col = C_RED
            elif key != "battery" and val > 80:
                col = C_RED
            c.create_text(x0, cy, text=label, fill="#6e6e6e", font=font_body(10), anchor="w")
            c.create_text(x0 + pw, cy, text=f"{val:.0f}{unit}", fill=col,
                          font=font_body_bold(10), anchor="e")
            cy += 13
            self._bar(c, x0, cy, pw, 5, val, col)
            cy += 20
        ...
        c.create_text(x0, cy, text=f"▲ {up_s}", fill="#5a5a5a", font=font_body(9), anchor="w")
        c.create_text(x0 + pw, cy, text=f"▼ {down_s}", fill="#5a5a5a", font=font_body(9), anchor="e")
        cy += 26
        ...
        # ── Saat ──────────────────────────────────────────────────────────
        focus_boost = self._focus_boost_for("time")
        time_col = "#ffffff" if focus_boost > 0.08 else "#d0d0d0"
        c.create_text(x0, cy, text=time.strftime("%H:%M:%S"), fill=time_col,
                      font=font_display(20), anchor="w")
        cy += 26
        c.create_text(x0, cy, text=time.strftime("%d %B %Y").upper(), fill="#6e6e6e",
                      font=font_body(10), anchor="w")
        cy += 26
        ...
        c.create_line(x0, cy, x0 + pw, cy, fill="#161616", width=1)
        cy += 20
        c.create_text(x0, cy, text=f"CLIMA · {self._weather_card['city'].upper()}",
                      fill="#6e6e6e", font=font_body_bold(9), anchor="w")
        cy += 18
        c.create_text(x0, cy, text=self._weather_card["primary"], fill="#e6e6e6",
                      font=font_body_bold(13), anchor="w")
        cy += 20
        for line in self._weather_card["details"][:2]:
            c.create_text(x0, cy, text=f"• {line}", fill="#8a8a8a",
                          font=font_body(9), anchor="w")
            cy += 16
        ...
        c.create_line(x0, cy, x0 + pw, cy, fill="#161616", width=1)
        cy += 20
        c.create_text(x0, cy, text="SAÚDE", fill="#6e6e6e", font=font_body_bold(9), anchor="w")
        cy += 18
        for line in self._health_card_lines[:2]:
            if cy > self.H - FOOTER_H - 130:
                break
            c.create_text(x0, cy, text=f"• {line}", fill="#8a8a8a",
                          font=font_body(9), anchor="w")
            cy += 16

    def _draw_gauge(self, c, cx, cy, r, pct, label):
        pct = max(0.0, min(100.0, float(pct)))
        track_w = 6
        c.create_oval(cx - r, cy - r, cx + r, cy + r,
                      outline="#1c1c1c", width=track_w)
        extent = -360.0 * (pct / 100.0)
        col = C_RED if pct > 85 else "#e6e6e6"
        if extent:
            c.create_arc(cx - r, cy - r, cx + r, cy + r,
                        start=90, extent=extent, style="arc",
                        outline=col, width=track_w)
        c.create_text(cx, cy - 8, text=label, fill="#6e6e6e", font=font_body_bold(9))
        c.create_text(cx, cy + 12, text=f"{pct:.0f}%", fill="#f2f2f2",
    ...
    def _draw(self):
        ...
        state_text = {"PAUSED": "Pausado", "LISTENING": "Listening", "SPEAKING": "Speaking",
                      "THINKING": "Thinking", "ERROR": "Error",
                      "INITIALISING": "Connecting"}.get(state_label, state_label.title())
        c.create_text(self.FCX, self.STATE_Y, text=state_text.upper(),
                      fill="#f0f0f0", font=(FONT_DISPLAY_FAMILY, 16, "bold"))
        self._draw_wave_dots(c, self.FCX, self.WAVE_Y, self.CTRL_W * 0.7, state_col)

        # ── Son yanıt kartı ──────────────────────────────────────────────────
        self._draw_response_card(c)

        # ── Mikrofon çevresi dekoratif dalga noktaları ──────────────────────
        mic_size = int(self._mute_canvas["width"])
        mic_cy = self.MIC_Y + self.MIC_R + 4
        self._draw_wave_dots(c, self.FCX - mic_size // 2 - 70, mic_cy, 120, "#5a5a5a", small=True)
        self._draw_wave_dots(c, self.FCX + mic_size // 2 + 70, mic_cy, 120, "#5a5a5a", small=True)

        # ── HEADER ───────────────────────────────────────────────────────────
        c.create_rectangle(0, 0, W, HDR_H, fill="#000000", outline="")
        c.create_line(24, HDR_H - 1, W - 24, HDR_H - 1, fill="#141414", width=1)

        # Sol üst: küçük ARIA wordmark
        c.create_text(24, 30, text=self._spaced_caps(SYSTEM_NAME),
                      fill="#e6e6e6", font=(FONT_DISPLAY_FAMILY, 15, "bold"), anchor="w")

        # Orta: büyük ARIA başlığı + alt yazı
        c.create_text(W // 2, 40, text=self._spaced_caps(SYSTEM_NAME),
                      fill="#ffffff", font=(FONT_DISPLAY_FAMILY, 34, "bold"))
        c.create_text(W // 2, 72, text=self._spaced_caps(MODEL_BADGE) if len(MODEL_BADGE) < 46
                      else " ".join(MODEL_BADGE.upper()),
                      fill="#6e6e6e", font=font_body(10))

        # Sağ üst: ONLINE hap rozeti
        ...
        self._draw_status_pill(c, W - 26, 30, indicator_text, ind_col)

        # ── FOOTER (ince, sade) ─────────────────────────────────────────────
        c.create_line(0, H - FOOTER_H, W, H - FOOTER_H, fill="#101010", width=1)
    ...
    def _show_setup_ui(self, edit_mode: bool = False):
        ...
        self.setup_frame = tk.Frame(self.root, bg="#050505",
        ...
        tk.Label(self.setup_frame, text="YOUTUBE API KEY",
                 fg=C_DIM, bg="#050505", font=font_body(12)).pack(pady=(10, 4))
        ...
        tk.Label(self.setup_frame, text="LM STUDIO MODEL ID",
                 fg=C_DIM, bg="#050505", font=font_body(12)).pack(pady=(6, 4))
        ...
        buttons = tk.Frame(self.setup_frame, bg="#050505")
        buttons.pack(pady=14)

        tk.Button(buttons, text="▸ KAYDET",
                  command=self._save_api_key, bg=C_BG, fg=C_PRI,
                  activebackground="#1e1e1e", font=font_body_bold(13),
                  borderwidth=0, padx=24, pady=10).pack(side="left", padx=8)

        if edit_mode:
            tk.Button(buttons, text="KAPAT",
                      command=self._close_setup_ui, bg="#0a0a0a", fg=C_DIM,
                      activebackground="#1a1a1a", font=font_body_bold(13),
                      borderwidth=0, padx=24, pady=10).pack(side="left", padx=8)
